...
(defun make-keyword (symbol)
  "Concatenate : onto front of symbol."
...
;;   "Saves lambda-list and lambda-expression when a function is defined.
;; Puts them in the symbol-plist of the function's name."
;;   `(progn
;;      (setf (get ',name 'function-lambda-list) ',lambda-list)
;;      (setf (get ',name 'function-lambda-expression) ',body)
...
;(defmacro letf (definitions &rest body)
;  "Behave as flet except takes function object instead of s-expression, and
;name can be used at beginning of a statement to execute."
;  (handler-case
;  `(let ((renamed-functions ()))
;     (loop for (fname fargs . fdata) in ',definitions do
;	  (when (fboundp fname)
;	    (setf (symbol-function (symbol-concat fname '-old))
;		  (symbol-function fname))
;	    (push fname renamed-functions))
;	  (eval (append (list 'defun fname fargs) fdata)))
;     ,@body
;     (loop for (fname fargs fdata) in ',definitions do
;	  (fmakunbound fname)
;	  (when (find fname renamed-functions)
...
;; 	   (magicffi:magic-file magic filespec))))
;;     (values mime (mapcar #'read-from-string (split mime "/")))))
